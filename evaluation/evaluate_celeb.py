'''
测评重构人脸在不同模型下的性能时，注意调整图像的归一化范围和模型训练数据一致：
...
class lfw_evaluator():
    def __init__(self, args, img_dir, targets_txt, encoder, metric='cosine', flip=True,device = None):
        ...
        with open(targets_txt, 'r') as fp:
            lines = fp.readlines()
        pos_ids = [l.strip()[:-11] for l in lines]#200
        # print('****************************',pos_ids)
        all_ids = os.listdir(img_dir)# 300
        neg_ids = list(set(all_ids) - set(pos_ids))#100
        pos_dirs = []
        for name in pos_ids:
            pos_dirs += glob.glob(os.path.join(img_dir, name, '*'))#3717
        neg_dirs = []
        for name in neg_ids:
            neg_dirs += glob.glob(os.path.join(img_dir, name, '*'))#2082        
        pos_set = lfw_dataset(pos_dirs, self.trf)#len=3717
        neg_set = lfw_dataset(neg_dirs, self.trf)#len=2082
    ...
    def compute_features(self, encoder, type, flip=True):
        ...
        if type == 'positive':
            ...
            features = torch.FloatTensor([])
            imnames = []
            for dir in self.att_dirs:#len(self.att_dirs)=200
                imname = dir.split('/')[-1]
    ...
    def attack_scores(self):
        ...
        """
        pos_names = ['_'.join(n.split('_')[:-1]) for n in self.pos_imnames]
        att_names = ['_'.join(n.split('_')[:-1]) for n in self.att_imnames]
        pos_names = np.array([pos_names])
        att_names = np.array([att_names])
        pos_imnames = np.array([self.pos_imnames])  # [1, 3717]
        att_imnames = np.array([self.att_imnames])  # [1, 200]
        mask_type1 = (att_imnames.T == pos_imnames) # [200, 3717]
        mask_type2 = (att_names.T == pos_names)     # [200, 3717]
...
def main(args):
    ...
    # _, img_dir = dataset_parser(args)
    with open(f'/media/Storage2/zh/face-privacy/Black-box-Face-Reconstruction_Hojin/dataset/dataset_conf.yaml') as fp:
    ...
    # with open('dataset/dataset_conf.yaml') as fp:
    #     conf = yaml.load(fp, Loader=yaml.FullLoader)
    #     conf = conf['lfw-200']
    # img_dir = conf['image_dir']
    # img_dir = img_dir + f'/{args.align}_aligned'
    ...
    pos_scores = evaluator.positive_scores()#13812372
