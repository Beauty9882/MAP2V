...
sys.path.append('/media/Storage2/zh/face-privacy/TFace-master/recognition/')
                ...
                if i < len(block) - 1:
                    modules.append(
                        unit_module(block[i].in_channel, block[i].depth,
                                    block[i].stride, feature_channels, kernel_size,device =device))
                else:
                    modules.append(
                        unit_module(block[i].in_channel, block[i].depth,
                                    block[i].stride, feature_channels, kernel_size, stage=stage, device =device, embedding=True))
            stage += 1
